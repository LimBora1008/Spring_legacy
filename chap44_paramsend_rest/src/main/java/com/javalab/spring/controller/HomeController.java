...
public class HomeController {
	...
	public String formRest(@ModelAttribute("member") MemberVo member) { // @ModelAttribute : 이동하는 화면에 넘겨줌
		return "formRest"; // 응답 페이지 : formRest.jsp
	}
	
	/*
	 * 응답 페이지 : formRest2.jsp
	 * 응답 페이지가 생략되면 호출된 url formRest2 가 반환되어
...
//		return "formRest2"; // 생략해도 응답 페이지 : formRest2.jsp
	}
	 ...
	 * 
	 * [@ResponseBody]
	 * 	- 호출한 곳으로 응답할때 "메시지 바디"에 담아서 전달해주면
	 * 		받는 jsp페이지 Body에 그대로 전달됨.
	 * 		자바스크립트에서 그걸 가공해서 사용함
	 * 	- <jackson-databind> 디펜던시(라이브러리)가 필요함
	 * 		Jackson은 Java 애플리케이션에서 JSON 데이터를 처리하는데
	 * 		널리 사용되며 JSON 데이터를 Java 객체로 변환(역직렬화)하고
	 * 		Java 객체를 JSON 데이터로 변환(직렬화)하는 기능을 제공
	 * 	- Map<String , Object>
	 * 		Map 타입으로 key-value 형태를 JSON Type 문자열로 반환
	 */
